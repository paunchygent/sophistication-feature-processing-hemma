...
def start_thread(def1, arg1, arg2, arg3):
	def guarded_run():
		try:
			...
			dataQueue.put("Error: " + type(exc).__name__ + ": " + str(exc))
			with open("/config/workspace/output/taaled-error.log", "w") as error_log:
				traceback.print_exc(file=error_log)

	t = Thread(target=guarded_run, daemon=True)
	t.start()
...
class MyApp: #this is the class for the gui and the text analysis
	def __init__(self, parent):
		...
		helv16= tkinter.font.Font(family= "Helvetica Neue", size = title_size, weight = "bold")
		...
		self.spacer1= tk.Label(parent, text= "TAALED · Lexical Diversity", font = helv16, background = color, pady = 12)
		self.spacer1.pack()
		...
		self.secondframe= tk.LabelFrame(self.myContainer1, text= "Files", background = color)
		...
		self.dirname = "/config/workspace/materials/data/ICNALE_500_merged_clean_texts/ICNALE_500_merged_clean"

		#This creates a label for the first program input (Input Directory)
		self.inputdirlabel =tk.LabelFrame(self.secondframe, height = "1", width= "45", padx = "4", text = "1. Input texts", background = color)
		self.inputdirlabel.pack(fill = tk.X)

		#Creates label that informs user which directory has been chosen
		directoryprompt = "Workshop Texts · 500 sample essays"
		...
		self.button1.configure(text= "Choose folder…")
		...
		self.outdirname = "/config/workspace/output/results.csv"
		...
		self.outputdirlabel = tk.LabelFrame(self.secondframe, height = "1", width= "45", padx = "4", text = "2. Results file", background = color)
		self.outputdirlabel.pack(fill = tk.X)

		#Creates a label that informs sure which directory has been chosen
		outdirectoryprompt = "Workshop Output · results.csv"
	...
	def button1Click(self, event):
		...
		if sys.version_info[0] == 2:
			import tkFileDialog
			self.dirname = tkFileDialog.askdirectory(parent=root,initialdir="/config/Navigation",title='Select input folder')

		if sys.version_info[0] == 3:
			import tkinter.filedialog
			self.dirname = tkinter.filedialog.askdirectory(parent=root,initialdir="/config/Navigation",title='Select input folder')
	...
	def button2Click(self, event):
		...
		self.outdirname = tkinter.filedialog.asksaveasfilename(parent=root, defaultextension = ".csv", initialdir="/config/workspace/output", initialfile = "results",title='Choose Output Filename')
	...
	def runprogram(self, event):
		...
		for key, value in self.var_dict.items():
			selected_options[key] = value.get() if hasattr(value, "get") else value
		start_thread(main, self.dirname, self.outdirname, selected_options)
